...
void GPIO_clk_Start(void) 
{
    ...
    GPIO_clk_CLKEN |= GPIO_clk_CLKEN_MASK;
	GPIO_clk_CLKSTBY |= GPIO_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: GPIO_clk_Stop
********************************************************************************
*
* Summary:
...
void GPIO_clk_Stop(void) 
{
    ...
    GPIO_clk_CLKEN &= (uint8)(~GPIO_clk_CLKEN_MASK);
	GPIO_clk_CLKSTBY &= (uint8)(~GPIO_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: GPIO_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((GPIO_clk_CLKEN & GPIO_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = GPIO_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = GPIO_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* GPIO_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(GPIO_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(GPIO_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: GPIO_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: GPIO_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = GPIO_clk_GetSourceRegister();
    uint16 oldDivider = GPIO_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = GPIO_clk_CLKEN & GPIO_clk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((GPIO_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: GPIO_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(GPIO_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: GPIO_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    GPIO_clk_MOD_SRC |= modeBitMask & (uint8)GPIO_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: GPIO_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    GPIO_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(GPIO_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: GPIO_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return GPIO_clk_MOD_SRC & (uint8)(GPIO_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: GPIO_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = GPIO_clk_GetDividerRegister();
    uint8 oldSrc = GPIO_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        GPIO_clk_MOD_SRC |= CYCLK_SSS;
        GPIO_clk_MOD_SRC =
            (GPIO_clk_MOD_SRC & (uint8)(~GPIO_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: GPIO_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return GPIO_clk_MOD_SRC & GPIO_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: GPIO_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    GPIO_clk_PHASE = clkPhase & GPIO_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: GPIO_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return GPIO_clk_PHASE & GPIO_clk_PHASE_MASK;
}
