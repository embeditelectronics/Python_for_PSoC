/*******************************************************************************
* File Name: DELSIG_ADC_Ext_CP_Clk.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_Start
********************************************************************************
*
* Summary:
...
void DELSIG_ADC_Ext_CP_Clk_Start(void) 
{
    ...
    DELSIG_ADC_Ext_CP_Clk_CLKEN |= DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK;
	DELSIG_ADC_Ext_CP_Clk_CLKSTBY |= DELSIG_ADC_Ext_CP_Clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_Stop
********************************************************************************
*
* Summary:
...
void DELSIG_ADC_Ext_CP_Clk_Stop(void) 
{
    ...
    DELSIG_ADC_Ext_CP_Clk_CLKEN &= (uint8)(~DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK);
	DELSIG_ADC_Ext_CP_Clk_CLKSTBY &= (uint8)(~DELSIG_ADC_Ext_CP_Clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((DELSIG_ADC_Ext_CP_Clk_CLKEN & DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* DELSIG_ADC_Ext_CP_Clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = DELSIG_ADC_Ext_CP_Clk_GetSourceRegister();
    uint16 oldDivider = DELSIG_ADC_Ext_CP_Clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = DELSIG_ADC_Ext_CP_Clk_CLKEN & DELSIG_ADC_Ext_CP_Clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
                DELSIG_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                DELSIG_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((DELSIG_ADC_Ext_CP_Clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR, clkDivider);
				DELSIG_ADC_Ext_CP_Clk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(DELSIG_ADC_Ext_CP_Clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    DELSIG_ADC_Ext_CP_Clk_MOD_SRC |= modeBitMask & (uint8)DELSIG_ADC_Ext_CP_Clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    DELSIG_ADC_Ext_CP_Clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(DELSIG_ADC_Ext_CP_Clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return DELSIG_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(DELSIG_ADC_Ext_CP_Clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = DELSIG_ADC_Ext_CP_Clk_GetDividerRegister();
    uint8 oldSrc = DELSIG_ADC_Ext_CP_Clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        DELSIG_ADC_Ext_CP_Clk_MOD_SRC |= CYCLK_SSS;
        DELSIG_ADC_Ext_CP_Clk_MOD_SRC =
            (DELSIG_ADC_Ext_CP_Clk_MOD_SRC & (uint8)(~DELSIG_ADC_Ext_CP_Clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return DELSIG_ADC_Ext_CP_Clk_MOD_SRC & DELSIG_ADC_Ext_CP_Clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    DELSIG_ADC_Ext_CP_Clk_PHASE = clkPhase & DELSIG_ADC_Ext_CP_Clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: DELSIG_ADC_Ext_CP_Clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return DELSIG_ADC_Ext_CP_Clk_PHASE & DELSIG_ADC_Ext_CP_Clk_PHASE_MASK;
}
